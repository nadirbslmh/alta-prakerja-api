...
type PlatformSignature interface {
	...
	GenerateForGenerateURL(ctx context.Context, t time.Time, input models.GenerateInput) (string, error)
}
...
func (p platformSignature) GenerateForGenerateURL(ctx context.Context, t time.Time, input models.GenerateInput) (string, error) {
	return p.generate(ctx, SignatureCheck{
		Method:        "POST",
		Timestamp:     t.Unix(),
		Authorization: "",
		Body:          []byte(fmt.Sprintf(`{"redeem_code":"%s","sequence":%d,"redirect_uri":"%s","email":"%s"}`, input.RedeemCode, input.Sequence, input.RedirectURI, input.Email)),
